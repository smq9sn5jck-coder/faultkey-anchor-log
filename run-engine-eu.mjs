/**
 * FaultKey Anchor-Log — eu-v1 batch (Stage B-EU)
 *
 * Loads cases/09–11 (the three EU enforcement matters), runs the SAME deterministic
 * scoring engine that ships in faultkey.com/try, applies the eu-v1 rule overlay
 * from /home/ubuntu/causallayer-mcp/demo/eu-rules.js, and writes one prediction
 * artifact per case to predictions-eu/<slug>.json. Then computes the eu-v1
 * aggregate root and writes anchor-payload-eu-v1.json — the file that gets
 * sealed to Sigstore Rekor + OpenTimestamps.
 *
 * Commitment scheme matches the v0.7 batch exactly:
 *   commitment_hash = sha256(canonicalJson({ case_id, input, prediction_core }))
 *   aggregate_root  = sha256(canonicalJson(sorted [{case_id, commitment_hash}]))
 *
 * The EU overlay is included in the prediction artifact (and therefore in the
 * commitment) under prediction.eu_overlay.
 */
...
import { applyEuRuleSet, RULE_SET_VERSION, euGateEngages } from "/home/ubuntu/causallayer-mcp/demo/eu-rules.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const CASES_DIR = join(__dirname, "cases");
const PRED_DIR = join(__dirname, "predictions-eu");
...
function deterministicScore(args) {
  ...
  return {
    ...
    _note: "Anchor-log prediction. Produced by the same deterministic scoring engine that runs at faultkey.com/try, with the eu-v1 rule overlay applied. Sealed by SHA256 commitment + OpenTimestamps + Sigstore Rekor.",
...
function applyEuOverlayToPrediction(input, prediction) {
  ...
  return applyEuRuleSet({ attributable, actors, flags });
}
...
for (const f of cases) {
  ...
  manifest.entries.push({
    ...
    commitment_hash, file: `predictions-eu/${scenario.case_id}.json`,
  });
...
writeFileSync(join(__dirname, "anchor-payload-eu-v1.json"), JSON.stringify(manifest, null, 2));
console.log(`\n eu-v1 manifest written: ${cases.length} predictions`);
console.log(` aggregate root: ${aggregate_root}`);

// Print SHA-256 of the literal file bytes — this is the value we will anchor to Rekor.
const fileBytes = readFileSync(join(__dirname, "anchor-payload-eu-v1.json"));
const fileHash = createHash("sha256").update(fileBytes).digest("hex");
console.log(` anchor-payload-eu-v1.json sha256 (Rekor-sealable): ${fileHash}`);
